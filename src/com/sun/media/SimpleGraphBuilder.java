 ...
 * 
 * It contains 3 parts: 1) Routines to search for all the supported output formats; 2) Routines to build a default flow
 * graph -- buildGraph;
 * 
 * A default graph is such that no customised option is specified on the TrackControl.
 * 
 * It operates on a breath-first search algorithm until the final target is reached as defined by the findTarget()
 * method. Intermediate search paths are stored as GraphNode's in the "candidates" vector.
 */
...
public class SimpleGraphBuilder {
    ...
    protected Vector<String> targetPluginNames = null;
     ...
     */
    public void reset() {
        Enumeration enum_ = plugIns.elements();
        GraphNode n;
        while (enum_.hasMoreElements()) {
            n = (GraphNode) enum_.nextElement();
            n.resetAttempted();
        }
    }
    ...
    /**
     * When the graph build finds a viable graph to build, this callback will be invoked to see if the graph can
     * actually be built. Subclass should implement this.
     */
        ...
        */
        ...
        for (int i = 0; i < outs.length; i++) {
            ...
            for (int j = 0; j < cnames.size(); j++) {
                ...
                // 2.1.1b hack	-ivg
                if (mp3Pkt && gn.plugin instanceof com.sun.media.codec.audio.mpa.DePacketizer)
                    continue;
    ...
    /**
     * This defines when the search ends. The "targets" array defines the nodes that are to be the "end points" (leaf
     * nodes) of the graph. With the default graph builder, the targets array contains the list of sinks that can
     * potentially support the input format.
     */
    ...
    boolean setDefaultTargetRenderer(Format in) {
        ...
        if (in instanceof AudioFormat) {
            targetPluginNames = PlugInManager.getPlugInList(new AudioFormat(null, AudioFormat.NOT_SPECIFIED,
                    AudioFormat.NOT_SPECIFIED, AudioFormat.NOT_SPECIFIED, AudioFormat.NOT_SPECIFIED,
                    AudioFormat.NOT_SPECIFIED, AudioFormat.NOT_SPECIFIED, AudioFormat.NOT_SPECIFIED, null), null,
                    PlugInManager.RENDERER);
        } else if (in instanceof VideoFormat) {
            targetPluginNames = PlugInManager.getPlugInList(new VideoFormat(null, null, VideoFormat.NOT_SPECIFIED,
                    null, VideoFormat.NOT_SPECIFIED // frameRate ???
    ...
    /**
     * Given a node that has failed, this function will eliminate it from the candiates list and mark it in the registry
     * as as failed node so it won't be attempted again. This method is upated by hsy on 10/10/2000. If we just remove
     * the failed node from candidates and mark it in the registy, we can't guarantee we could roll back to the
     * exact/desired node/state to re-start searching, since this node might have already been removed from candidates.
     * The simple/straightforward fix is to clear up candidates and plugIns and start allover again.
     */
    void removeFailure(Vector<GraphNode> candidates, GraphNode failed, Format input) {
        ...
        /****
         * This is the old implementation. GraphNode n; Enumeration enum = candidates.elements(); while
         * (enum.hasMoreElements()) { n = (GraphNode)enum.nextElement(); if (n.plugin == failed.plugin)
         * candidates.removeElement(n); }
         * 
         * if ((n = (GraphNode)plugIns.get(failed.plugin.getClass().getName())) != null) n.failed = true;
         ******/
    }

    /**
     * Given a codec class name, instantiate the codec and query it dynamically to see if it supports the given input
     * and output formats.
     */
    static public GraphNode getPlugInNode(String name, int type, Hashtable<String, GraphNode> plugIns) {
        GraphNode gn = null;
    ...
    /**
     * Find a codec that can handle the given input and output. The output argument can be null if no specific output
     * format is required.
     */
    ...
    /**
     * Find a renderer that can handle the given input and output. The output argument can be null if no specific output
     * format is required.
     */
    ...
    /**
     * Return a chain of codecs and renderer to render to input format. Unlike findCodec and findRenderer, it uses the
     * same graph building algorithm that the media engine uses to determine the best rendering path for a particular
     * input format. The return value is a vector of plugins of all the codecs and the renderer. The plugin list is in
     * reverse order starting from the renderer. The list of the corresponding input formats for each codec is also
     * returned as an argument to the function.
     */
    static public Vector findRenderingChain(Format in, Vector<Format> formats) {
    ...
    static public PlugIn createPlugIn(String name, int type) {
        ...
        try {
            ...
            // START OF MODIFICATION (Samuel Berner)
            // if Swing enabled renderer: create in Swing Event Dispatch thread
            if (type == PlugInManager.RENDERER && JComponent.class.isAssignableFrom(cls)) {
                final Object[] swingComponent = new Object[1];
                SwingUtilities.invokeAndWait(new Runnable() {
                    @Override
                    public void run() {
                        try {
                            swingComponent[0] = cls.newInstance();
    ...
    /**
     * Choose a format among the two input arrays that matches and verify that if the given upstream and downstream
     * plugins accept the matched format as output (for the upstream) or as input (for the downstream). Either of the
     * plugin arguments can be null. In which case the verification step will be skipped accordingly.
     * 
