# Figurabia
A Swing GUI Application that can show dance videos and allows to "index" them to extract the positions of the dancers and be able to link between different videos with the same dancer positions. This allows a dancer to browser their videos for alternative moves in any specific position. It also allows to see one's collection of videos of dance moves visualized as a graph network.

Figurabia has three modes:
- Figure Editor
- Figure Explorer
- Figure Mapper
...
![Figure Editor Activate Figure](readme-images/figurabia-figure-editor-activate.png)
...
When the video moves or the user selects a different position in the video, the information about how the dancers are positioned gets transfered into the left upper view. Here the user can change the view and it will not affect the current video but instead affect the view in the bottom which displays all "connecting moves" of all videos.
